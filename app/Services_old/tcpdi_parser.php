...
require_once('tcpdf_filters.php');
...
// -----------------------------------------------------------------------------
    ...
    protected function decodeXrefStream($startxref, $xref=array()) {
        ...
        foreach ($keys as $k=>$key) {
            ...
            elseif (($key == '/Index') AND ($v[0] == PDF_TYPE_ARRAY AND $this->counter($v[1]))) {
    ...
    private function counter($v)
    {
        $count = count(array_filter($v, function ($element) {
            return $element >= 2;
        }));
