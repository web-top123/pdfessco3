    ...
    <modal @close="$emit('close')" class="document-created-confirmation">
        ...
        <template>
            ...
            <div class="field is-grouped btwn">
                <a class="is-link send-email-link" :class="{ 'on-active': emailVisible }"
                    @click="emailVisible = !emailVisible"><i class="fa fa-envelope-o"></i>Send by Email<span
                        class="border-send-email"></span></a>
            ...
            <div class="send-by-email" v-if="emailVisible">
                ...
                <div class="field send-email-field">
                    <label class="label is-small" for="email-input"> SEND EMAIL TO (use ; between multiple email addresses
                        when sending)</label>
                    <div class="control">
                        <textarea type="email" id="email-input" ref="txa" class="input is-medium send-by-email-input"
                            :class="{ 'is-danger': errors.emailList.length > 0, 'is-success': successState === true, 'bounce-enter-active': failState === true }"
                            v-model="emailString" placeholder="john@domain.com; chris@domain.com"
                            @keyup="checkEmail"></textarea>
                        <p class="help is-danger error-message-flash" :class="{ 'bounce-enter-active': failState === true }"
                            v-if="errors.emailList.length > 0">
                ...
                <button class="button is-info send-by-email-button" :class="{ 'is-success': successState === true }">
                    <transition name="fade">
                        <span class="save-button" v-if="sending === false" key="editing" @click="sendByEmail">Send
                            Email</span>
                        <span class="save-button" v-if="sending === true" key="saved">Sending Email</span>
                    </transition>
                </button>
                <div class="email-sent-success" :class="{ 'bounce-enter-active': emailSuccess === true }"
                    v-if="emailSuccess">
                    Your email has been successfully sent! <span class="fa fa-times-circle clear-confirmation"
                        @click="emailSuccess = false;"></span></div>
    ...
    methods: {
        ...
        async sendByEmail() {
            ...
            if (!this.emailString.length) {
                ...
                console.log('url', "xx");
                console.log('urls', this.filePath);
