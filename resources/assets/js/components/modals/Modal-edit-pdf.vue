...
<modal @close="$store.commit('manageFiles/closeModal', name)" class="move-modal">
    ...
    <template slot="header">
            <p slot="header" class="modal-card-title has-text-centered">Edit File</p>
    </template>

    <template>
        <div class="form-group">
            <label>File Name</label>
            <input v-model="pdf.name" type="text" placeholder="Name..." :class="{error: errors.name}" @keyup="errors.name = false">
        </div>

        <div class="form-group">
            <label>Description</label>
            <textarea v-model="pdf.description" rows="6" placeholder="Description..." :class="{error: errors.description}" @keyup="errors.description = false"></textarea>
        </div>

        <p class="uploaded-by">Uploaded by <span>{{pdf.user.name}}</span> at <span>{{pdf.created_at}}</span></p>
