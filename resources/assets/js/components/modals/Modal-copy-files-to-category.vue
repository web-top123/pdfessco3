...
export default {
    ...
    mounted() {
        ...
        axios.post('/admin/copy-to/list', { categories: this.selectedArr.categories }).then(({ data }) => this.listLimit = this.list.map(item => Object.assign({ '$isDisabled': item.depth > 30 }, item)).filter(item => this.$store.state.manageFiles.selected.categories.indexOf(item.id) < 0));
    },
    methods: {
        save() {
